import Database from'better-sqlite3';import fs from'node:fs';import path from'node:path';import{fileURLToPath}from'node:url';
const dir=path.join(path.dirname(fileURLToPath(import.meta.url)),'../data');fs.mkdirSync(dir,{recursive:true});export const db=new Database(path.join(dir,'dasketball.db'));db.pragma('journal_mode = WAL');
db.exec(`CREATE TABLE IF NOT EXISTS players(id INTEGER PRIMARY KEY AUTOINCREMENT,username TEXT UNIQUE NOT NULL,password TEXT NOT NULL,createdAt TEXT DEFAULT CURRENT_TIMESTAMP,coins INTEGER DEFAULT 1000,wins INTEGER DEFAULT 0,losses INTEGER DEFAULT 0,shooting INTEGER DEFAULT 72,speed INTEGER DEFAULT 74,defense INTEGER DEFAULT 68,dribbling INTEGER DEFAULT 76,strength INTEGER DEFAULT 65,stamina INTEGER DEFAULT 75,hair TEXT DEFAULT 'fade',height REAL DEFAULT 6.4,clothing TEXT DEFAULT 'purple',build TEXT DEFAULT 'Athletic',gamesPlayed INTEGER DEFAULT 0,difficulty TEXT DEFAULT 'Pro',championshipsWon INTEGER DEFAULT 0,tutorialDone INTEGER DEFAULT 0);
